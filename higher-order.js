...
//Code Here
let totalPopulation   = populations.reduce((acc, x)=>acc+x)
totalPopulation
...
let ordersTotal = orders.map((x)=>(Number(x.price)+ Number(x.tax)))
// let ordersTotal = orders.reduce(function (previous, current) {
...
// let bobsTotal = purchases.reduce(function (acc, x) {
//   return acc + x.price;
// }, 0);
//.reduce((acc, x)=> {return acc + x.price;))
let bobsTotal = purchases.filter(x=> x.owner ==="Bob")
bobsTotal = bobsTotal.reduce((acc, x)=> {return acc+  x.price}, 0)
bobsTotal
